...
unsigned long programEnd    = 0x07FF; // Program area
unsigned long configStart   = 0x2000;
unsigned long configEnd     = 0x2007;
unsigned long dataStart     = 0x2100; // EEPROM area
unsigned long dataEnd       = 0x217F; // EEPROM area
...
struct deviceInfo
{
    PGM_P name;      // User-readable name of the device.
...
void initDevice(const struct deviceInfo *dev)
{
    ...
    Serial.print("DeviceName: "); // strChannelBuffer is determined by these messages to the console
    printProgString((PGM_P)(pgm_read_word(&(dev->name))));
    Serial.println();
    ...
    Serial.println();
    Serial.print("FlashType: ");
    printHex8(progFlashType);
    Serial.println();
}
...
void cmdErase(const char *args)
{
    ...
    if (preserve && reservedStart <= reservedEnd) { // 0001 & 03FF <= 03FF
        size_t size = ((size_t)(reservedEnd - reservedStart + 1)) // 0002
            * sizeof(unsigned int);
    ...
    Serial.println();
    Serial.print("GetReservedWord: ");
    //printHex8(configSave); // 3000
    printHex8(configWord); // 0FFF
    Serial.println();
    ...
    if (reserved) {
        ...
        while (addr <= reservedEnd) {
            ...
            //if (!writeWordForced(addr, reserved[offset]))
                ok = false;
            ++addr;
            ++offset;
        }
        /*Serial.println();
        Serial.print("GetReservedWord: ");
        printHex8(reserved[0]);
        Serial.println();*/
        /*if (!writeWord(addr, reserved[0])){
            ok = false;
        }*/
        free(reserved);
        if (!ok) {
